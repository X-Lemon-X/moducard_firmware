...
public:
  Result<std::shared_ptr<McSlaveDriver<
      McCanSlaveInterface, Hardware>>> static Make(std::shared_ptr<CanBase>
                                                       can_interface,
                                                   uint32_t uid_21_bit) {
    if (!can_interface) {
  ...
  Status control_loop() {
    ...
    case DeviceMode::NORMAL:
      return mode_normal_loop();
      break;
    case DeviceMode::CONFIGURATION:
      return mode_configuration_loop();
      break;
    default:
      break;
    }
...
private:
  ...
  Status generic_switch_mode(DeviceMode new_mode) {
    ...
    case DeviceMode::NORMAL:
      ...
      _mode_enter_mode_func =
          std::bind(&McSlaveDriver::mode_enter_normal, this);
      break;
    case DeviceMode::CONFIGURATION:
      _mode_exit_mode_func =
          std::bind(&McSlaveDriver::exit_configuration_mode, this);
      _mode_enter_mode_func =
  ...
  Status mode_enter_normal() {
    ...
    ARI_RETURN_ON_ERROR(_can_interface->add_callback(
        ...
        nullptr));
    ARI_RETURN_ON_ERROR(_can_interface->add_callback(
        mcan_connect_msg_id_with_node_id(configs::PingModule::k_base_address,
                                         _node_id, true),
    ...
    std::apply(
        [&](auto &&...args) {
          (
              [&] {
                using MsgT =
                    std::decay_t<decltype(this->_interface.*(args.second))>;
                auto msg_buffer =
                    std::make_shared<CanMultiPackageFrame<MsgT>>();
                (void)_can_interface->add_callback(
                    mcan_connect_msg_id_with_node_id(
                        (this->_interface.*(args.second)).k_base_address,
                        _node_id),
                    [this, args, msg_buffer](CanBase &cd, const CanFrame &frame,
                                             void *ar) {
        ...
        [&](auto &&...args) {
          ((void)_can_interface->add_callback(
               ...
               [this, args](CanBase &cd, const CanFrame &frame, void *ar) {
                 (void)mcan_pack_send_msg(*_can_interface,
                                          this->_interface.*(args), _node_id);
               }),
           ...);
        },
  ...
  Status mode_exit_normal() {
    ...
    ARI_RETURN_ON_ERROR(
        _can_interface->remove_callback(mcan_connect_msg_id_with_node_id(
            configs::GetHardwareType::k_base_address, _node_id, true)));
  ...
  void callback_get_hardware_type(CanBase &cd, const CanFrame &frame,
                                  void *args) {
    ...
    std::memcpy(response_frame.data, reinterpret_cast<uint8_t *>(&response),
                sizeof(response));
    cd.send(response_frame);
  }

  // void callback_nm_set_device_node_id(CanBase &cd, const CanFrame &frame,
  // void *args);
  void callback_nm_enter_configuration_mode(CanBase &cd, const CanFrame &frame,
                                            void *args) {
  ...
  void callback_flash_indicator_led(CanBase &cd, const CanFrame &frame,
                                    void *args) {
  ...
  Status enter_configuration_mode() {
    ...
    ARI_RETURN_ON_ERROR(_can_interface->add_callback(
        ...
        nullptr));
    ARI_RETURN_ON_ERROR(_can_interface->add_callback(
        mcan_connect_msg_id_with_node_id(_uid_21_bit, 1),
  ...
  void callback_discover_devices(CanBase &cd, const CanFrame &frame,
                                 void *args) {
  ...
  void callback_set_device_node_id(CanBase &cd, const CanFrame &frame,
                                   void *args) {
  ...
  void callback_enter_configuration_mode(CanBase &cd, const CanFrame &frame,
                                         void *args) {
    (void)(generic_switch_mode(DeviceMode::CONFIGURATION));
  }
